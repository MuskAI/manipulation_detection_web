...
def focal_loss_fixed(y_true, y_pred, gamma=2., alpha=.25):
    ...
    cost1 = -K.sum(alpha * K.pow(1. - pt_1, y_all) * K.log(K.epsilon() + pt_1)) - K.sum(
        (1 - alpha) * K.pow(pt_0, y_all) * K.log(1. - pt_0 + K.epsilon()))
...
def huber_loss(y_true, y_pred, delta=1):
    return tf.losses.huber_loss(labels=y_true, predictions=y_pred, delta=delta)
...
def wce_huber_loss(y_true, y_pred, e1=0.45, e2=0.05, delta=0.2):
    loss1 = cross_entropy_balanced(y_true, y_pred)
    loss2 = huber_loss(y_true, y_pred, delta=delta)
    loss3 = dice_coef_loss(y_true, y_pred)
    return (1 - e1 - e2) * loss1 + e1 * loss2 + e2 * loss3


def wce_huber_loss_eight(y_true, y_pred, e1=0.5, delta=0.5):
    loss1 = cross_entropy_balanced(y_true, y_pred)
    loss2 = huber_loss(y_true, y_pred, delta=delta)
    return e1 * loss1 + (1 - e1) * loss2


def wce_huber_loss_new(y_true, y_pred, e1=0.7, delta=0.5):
    loss1 = cross_entropy_balanced(y_true, y_pred)
    loss2 = huber_loss(y_true, y_pred, delta=delta)
    return e1 * loss1 + (1 - e1) * loss2
